...
# local-frame/collide/setPos telemetry. No movement, collision, input, world,
# train, camera, ownership, or physics value is changed.
s = probe.read_text(encoding="utf-8")
old = '"GATE_E_CLIENT_STATE player_pos={},{},{} player_box='
new = '"GATE_E_CLIENT_STATE player_tick={} player_pos={},{},{} player_box='
if old not in s:
...
for text, forbidden in [
    ...
    if forbidden not in text:
        raise SystemExit("ceiling trace verification failed: " + forbidden)

print("M1_CEILING_CONTACT_CORRELATION_TRACE prepared=true read_only=true local_frame=create_worldToLocalPos gameplay_mutated=false collision_mutated=false input_mutated=false camera_mutated=false")

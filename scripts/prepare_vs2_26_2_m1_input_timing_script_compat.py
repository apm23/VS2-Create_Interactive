...
if old_wall_prefix not in source:
    # Idempotent when the coordinate-agnostic compatibility patch is already present.
    if "M1_CURRENT_COORDINATE_AGNOSTIC_WALL_COMPOSER" in source:
        print("M1 input composer source compatibility already applied")
        raise SystemExit(0)
    raise SystemExit("M1 input composer source compatibility could not find legacy coordinate-specific wall block")

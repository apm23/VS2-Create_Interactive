...
# broadphase. Production-world #530 additionally proves a current/previous-tick exact-carriage
# native application must bridge a transient strict physical-support timing seam while grounded;
# otherwise contact refresh arrives only after the player has already left the moving frame.
# This only calls Create's registerColliding; it does not synthesize position, velocity, gravity,
# collision response, or a carry vector. The 20-tick age ceiling prevents stale carriage contact
# from surviving an unrelated long fall.
...
print("Phase 85: retains exact-carriage native Create contact through grounded support seams and one bounded airborne arc; legacy replay remains disabled")
